...
def train_and_evaluate(config_path):
    ...
    model_dir = config['Anemia']['model_path']
    rf_params = config['Anemia']['RandomForestClassifier']['params']

    print(f"📊 Splitting data (test size = {split_ratio})...")
    features = df.drop(columns=['diseased'], axis=1)
    target = df['diseased']
    ...
    mlflow_config = config['Anemia']['mlflow_config']
